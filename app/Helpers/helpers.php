...
if (!function_exists('generateColumnsPermissions')) {
    function generateColumnsPermissions($array, $number, $user, $show = false)
    {
        $count = 1;
        $html = "<div class='row'>";
        foreach ($array as $index => $item){
            ...
            if(!$show){
                ...
                $checked = ($user->hasPermissionTo($item->name)) ? 'checked' : '';
                $html .= <<<"EOT"
                            <input class="form-check-input" type="checkbox" name="permissions[]"
                                id="permission_$index" value="$item->name" $checked>
                        EOT;
            }else{
                $html .= '<div>';
...
if (!function_exists('getTypeDivorce')) {
    function getTypeDivorce($id){
        $response = App\Models\TypeDivorce::find($id)->name;
        return $response;
    }
}
